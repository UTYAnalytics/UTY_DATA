...
def add_columns_to_table(df: pd.DataFrame, api: KeepaAPI) -> pd.DataFrame:
    ...
    # sellers_info = api.sellers(seller_ids).get("sellers")

    # Initialize new columns
    df["totalStorefrontAsins"] = None
    df["ratingCount"] = None

    for index, row in df.iterrows():
        seller_id = row["Seller Id"]

        # Retrieve seller data from the API response
        seller_data = api.sellers([seller_id]).get("sellers").get(seller_id)
        if seller_data:
            total_storefront_asins = seller_data.get("totalStorefrontAsins", [0, 0])[1]
            rating_count = seller_data.get("ratingCount", [0, 0, 0, 0])[3]

            # Update the DataFrame with new data
            df.at[index, "totalStorefrontAsins"] = total_storefront_asins
